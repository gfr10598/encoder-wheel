...
class TestFrustumInnerFacePlacement(unittest.TestCase):
    ...
    def test_step_by_step(self):
        ...
        print(
            f"  z_inner (cell)  = {z_inner_cell:.4f} mm  (= H/2 - taper_in = {H/2:.3f} - {taper_in:.3f})"
        )
        ...
        self.assertAlmostEqual(
            top_z, m["axial_mm"] / 2, places=2, msg="magnet top not at axial_mm/2"
        )
        ...
        print(
            f"[2] offset(cl/2={cl/2:.3f})        top edge z = {top_z_cl:.4f}  (expect ≈{expected_top_cl:.4f})"
        )
        self.assertAlmostEqual(
            top_z_cl,
            expected_top_cl,
        ...
        print(
            f"[3] bisect at z_inner={z_inner:.3f}  top face z = {cs_face_z:.4f}  (expect {z_inner:.4f})"
        )
        self.assertAlmostEqual(
            cs_face_z, z_inner, places=2, msg="bisect face not at z_inner"
        )
        ...
        w_outer = rect.fillet_2d(r_out, rect.vertices()).moved(
            Location((0, 0, z_outer))
        )
        taper_frustrum = Solid.make_loft([w_inner, w_outer])

        bot_faces = [
            f for f in taper_frustrum.faces() if abs(f.center().Z - z_inner) < 0.1
        ]
        self.assertTrue(bot_faces, "No bottom face at z_inner in taper_frustrum")
        frust_bot_z = max(f.center().Z for f in bot_faces)
        print(
            f"[4] frustum lofted        bottom face z = {frust_bot_z:.4f}  (expect {z_inner:.4f})"
        )
        self.assertAlmostEqual(
            frust_bot_z, z_inner, places=2, msg="frustum bottom face not at z_inner"
        )
        ...
        print(
            f"[5] fused cutter          {len(step_edges)} step edges  mean z = {mean_fused:.4f}  (expect {z_inner:.4f})"
        )
        self.assertAlmostEqual(
            mean_fused, z_inner, places=1, msg="fused cutter step not at z_inner"
        )
        ...
        print(
            f"[6] placed +insertion_Z   {len(step_cell)} step edges  mean z = {mean_cell:.4f}  (expect {z_inner_cell:.4f})"
        )
        self.assertAlmostEqual(
            mean_cell,
            z_inner_cell,
        ...
        if step_tilted:
            ...
            print(
                f"[7] after tilt — no step edges found near z={z_inner_cell:.3f} (tol=±0.5)"
            )

        print(f"\n── Summary ─────────────────────────────────────────────────────")
        print(
            f"  Formula:  H/2 - taper_in = {H/2:.4f} - {taper_in:.4f} = {z_inner_cell:.4f} mm"
        )
